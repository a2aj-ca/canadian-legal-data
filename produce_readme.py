...
def load_coverage(path: str) -> Dict[str, Any]:
    """Load coverage data from a local JSON file."""
    try:
...
def generate_readme():
    ...
    # Load coverage data from local JSON files
    cases_data = load_coverage("UTILS/case_coverage.json")
    laws_data = load_coverage("UTILS/laws_coverage.json")

    today = datetime.now().strftime("%Y-%m-%d")

    # Process cases data (preserve order from JSON file)
    case_table_rows = []
    total_cases = 0

    for item in cases_data.get("results", []):
    ...
    # Process laws data (preserve order from JSON file)
    law_table_rows = []
    total_laws = 0

    for item in laws_data.get("results", []):
        dataset = item["dataset"]
        count = item["number_of_documents"]
        total_laws += count
        first_date = format_date(item["earliest_document_date"])
